...
from kanjivg_ml.check import isGeneralKanji
...
def parse_kanji_svg(filename):
    ...
    for i in range(len(path_data) + 1):
        ...
        if kanji_flg:
            path = pathlib.Path('./output/kanji/svg/')
            path_png = pathlib.Path('./output/kanji/png/')
            path_png_w = pathlib.Path('./output/kanji/png_white/')
        else:
            path = pathlib.Path('./output/other/svg/')
            path_png = pathlib.Path('./output/other/png/')
            path_png_w = pathlib.Path('./output/other/png_white/')
